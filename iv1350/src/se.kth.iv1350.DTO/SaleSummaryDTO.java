...
public record SaleSummaryDTO(String latestItemName, String latestItemDescription, Amount latestItemPrice,
              VAT latestItemVAT, int latestItemID, Amount runningTotal, Amount totalVAT) {
    /**
    ...
    public SaleSummaryDTO(ItemInBasketDTO itemInBasketDTO, Amount runningTotal, Amount totalVAT) {
        this(itemInBasketDTO.name(), itemInBasketDTO.description(), new Amount(itemInBasketDTO.price()),
                itemInBasketDTO.vatRate(), itemInBasketDTO.itemID(), new Amount(runningTotal), new Amount(totalVAT));
    }
}